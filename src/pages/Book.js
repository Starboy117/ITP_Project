...
const BookingDetails = () => {
  ...
  const [phone, setPhone] = useState(
    currentUser?.phone?.replace(/^(\+94|0)/, "") || ""
  );
  ...
  const validateForm = () => {
    ...
    else if (!/^\+94\d{7,12}$/.test(fullPhone))
      newErrors.phone = "Phone must start with +94 and be 10-15 digits.";
  ...
  const handleSubmit = async (e) => {
    ...
    const formattedPhone = `+94${phone}`; // ensure +94 prefix
    const data = {
          ...
          <form className="grid grid-cols-1 gap-6" onSubmit={handleSubmit}>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              <div>
                ...
                <div className="flex">
                  <span className="inline-flex items-center px-3 rounded-l-lg bg-gray-700 text-white">
                    +94
                  </span>
                  <input
                    type="tel"
                    ...
                    className={`flex-1 p-3 rounded-r-lg bg-gray-900 border ${
